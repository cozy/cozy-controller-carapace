...
module.exports._listen2 = function overrideNet() {
  ...
  net.Server.prototype._listen2 = function ourListen(address, port, addressType, desired) {
    ...
    bind_ip = carapace.cli.argv().bind_ip
    address = '127.0.0.1';
    port = toPort(port);
    if (process.env.NODE_ENV === 'development') {
      address = '0.0.0.0';
    }
    if (process.env.NAME === "proxy" && bind_ip){
      address = bind_ip;
    }
    ...
    function onError(err) {
      ...
      if (err.code !== 'EADDRINUSE' && err.code !== 'EACCES') {
        ...
        ourListen.call(self, '127.0.0.1', nextPort(port), addressType, desired);
      }
    }
